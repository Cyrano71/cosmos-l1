...
export default function useExampleExampleV_1() {
  const client = useClient();

  type QueryParamsMethod = typeof client.ExampleExampleV_1.query.queryParams;
  ...
  const QueryParams = ( options: Partial<UseQueryOptions<QueryParamsData>>) => {
    ...
    return useQuery<QueryParamsData>({ queryKey: [key], queryFn: async () => {
      const res = await client.ExampleExampleV_1.query.queryParams();
        return res.data;
    }, ...options});
  }
